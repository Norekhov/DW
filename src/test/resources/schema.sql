...
(
    ad_pk     INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    price     integer,
    title     varchar(255),
    ad_text   varchar(255),
...
(
    comment_pk INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at bigint,
    text       varchar(255),
    ad_pk      integer,
        foreign key (ad_pk) references ad(ad_pk),
    user_id    integer,
        foreign key (user_id) references users(user_id)
);